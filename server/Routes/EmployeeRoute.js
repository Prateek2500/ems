        ...
        if (result.length > 0) {
            bcrypt.compare(req.body.password, result[0].password, (err, match) => {
                ...
                const token = jwt.sign(
                    { role: "employee", email: result[0].email, id: result[0].id, name: result[0].name },
                    "jwt_secret_key",
                    { expiresIn: "1d" }
                );
...
router.get('/:id', (req, res) => {
    ...
    pool.query(sql, [req.params.id], (err, result) => {
        ...
        if (result.length > 0) {
            return res.json({ Status: true, Result: { ...result[0], dept_id: result[0].dept_id?.toString() } });
        } else {
...
router.post("/add_employee", (req, res) => {
    bcrypt.hash(req.body.password, 10, (err, hash) => {
        ...
        `;
        const values = [
            req.body.name, req.body.email, hash, req.body.salary, req.body.address,
            req.body.dept_id, req.body.image || "", req.body.age, req.body.gender,
            req.body.account_no, req.body.bank_name, req.body.branch, req.body.university,
            req.body.degree, req.body.edu_branch, req.body.gradepoint, req.body.yop,
            req.body.father_name, req.body.mother_name, req.body.emergency_contact,
            req.body.alternate_contact, req.body.aadhar_number, req.body.pan_number
        ];
...
router.post('/attendance', (req, res) => {
    ...
    jwt.verify(token, "jwt_secret_key", (err, decoded) => {
        ...
        pool.query("SET time_zone = '+05:30';", () => {
            const sql = `
                INSERT INTO attendance (employee_id, attendance_date, attendance_time, status)
                VALUES (?, CURDATE(), CURTIME(), ?)
                ON DUPLICATE KEY UPDATE 
                    status = ?, attendance_time = CURTIME()
            `;
            pool.query(sql, [decoded.id, status, status], (err) => {
                if (err) return res.status(500).json({ Status: false, Error: "Database error" });

                const todayIST = new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" });
                return res.json({ Status: true, Message: "Attendance recorded", Record: { employee_id: decoded.id, date: todayIST, status } });
            });
        });
    });
});
        ...
        jwt.verify(token, "jwt_secret_key", (err, decoded) => {
            ...
            pool.query(sql, [decoded.id], (err, result) => {
                ...
                return res.json({ 
                    Status: true, 
                    Result: result.map(r => ({
                        date: new Date(r.attendance_date).toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" }),
...
router.delete('/attendance/reset', (req, res) => {
    pool.query("SET time_zone = '+05:30';", () => {
        ...
        jwt.verify(token, "jwt_secret_key", (err, decoded) => {
            ...
            const sql = "DELETE FROM attendance WHERE employee_id = ? AND attendance_date = CURDATE()";
            pool.query(sql, [decoded.id], (err) => {
                if (err) return res.status(500).json({ Status: false, Error: "Database error" });
                return res.json({ Status: true, Message: "Today's attendance reset" });
            });
        });
    });
});
...
router.post('/apply_leave', (req, res) => {
    ...
    jwt.verify(token, "jwt_secret_key", (err, decoded) => {
        ...
        const sql = `
            ...
            VALUES (?, ?, ?, ?, ?, 'pending')
        `;
        pool.query(sql, [decoded.id, start_date, end_date, leave_type, purpose], (err, result) => {
            if (err) return res.json({ Status: false, Error: err.message });
            return res.json({ Status: true, Result: result });
        });
    });
